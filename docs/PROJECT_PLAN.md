...
- [x] Render Free versus Premium scanner entitlement/limit information from the authenticated API quota endpoint; server remains the source of truth. (production build passed)
  - [x] Align landing-page scan-access copy with the server quota: sign-in is required for the two successful Free scans per day, while Premium remains unlimited. (lint:ci + 40 Vitest tests + production build passed)
...
- [ ] Add component/integration tests for auth, scanner and community states.
	- [x] Add jsdom Scanner-page regression coverage for malformed API data and malformed direct EVM input, ensuring the page stays on its localized, recoverable retry path instead of crashing. (targeted Vitest + lint + production build passed)
  ...
  - [x] Normalize optional post presentation fields and reject incomplete post-detail payloads before News/Post detail render them, so one malformed crawler/API record cannot crash the route. (targeted Vitest + lint + production build passed)
...
- [x] Validate finite numeric Binance fields before prices, candles or ticker changes reach render state, preventing malformed provider payloads from displaying `NaN`. (Vitest + lint + production build passed)
  - [x] Keep Analysis market-list rows in an explicit unavailable state until a validated Binance ticker arrives, rather than presenting provider delays as a misleading `$0.00` price or `0.00%` change. (targeted Vitest + lint + production build passed)
...
- [x] Guard shared date formatting against malformed crawler/API timestamps, returning a localized unavailable state instead of allowing date rendering to crash a route. (lint:ci + Vitest + production build passed)
  - [x] Guard Community's relative-time renderer against malformed API timestamps so it cannot display `NaN` time labels. (targeted Vitest + lint + production build passed)
...
- [x] Keep Header notification loading independent from the VN/EN render state, so changing language does not trigger an unnecessary notification API request. (targeted lint + Vitest + production build passed)
  ...
  - [x] Run lint, Vitest and production build on every pull request and push to `main`; Community is now included in the lint baseline. (global lint + Vitest + production build passed)
