    ...
    try {
      ...
      setError(
        getErrorMessage(
          ...
          translate(
            language,
            "Không tải được danh sách group. Vui lòng thử lại.",
    ...
    <main className="min-h-[calc(100vh-12rem)] px-4 py-8 sm:px-6 lg:py-12">
      <section className="mx-auto max-w-5xl">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold text-white sm:text-4xl">
              {translate(language, "Group nhà đầu tư", "Investor groups")}
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">
              {translate(
                language,
                "Thảo luận tập trung theo chủ đề, cùng quy tắc thành viên và luồng bài viết riêng.",
                "Focused discussion spaces with member rules and their own post feeds.",
              )}
            </p>
          </div>
          {!signedIn ? (
            <Link
              href="/login"
              className="inline-flex items-center justify-center rounded-xl bg-sky-500 px-4 py-2.5 text-sm font-semibold text-slate-950 hover:bg-sky-400"
            >
              {translate(
                language,
                "Đăng nhập để tạo group",
            ...
            >
              ...
              {translate(language, "Tạo group", "Create group")}
            </button>
          ) : entitlement === "loading" ? (
            <span className="inline-flex items-center gap-2 rounded-xl border border-slate-700 px-4 py-2.5 text-sm font-semibold text-slate-400">
              <Loader2 className="h-4 w-4 animate-spin" />
              {translate(
                language,
                "Đang kiểm tra Premium…",
                "Checking Premium…",
              )}
            </span>
          ) : entitlementError ? (
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-amber-400/35 bg-amber-500/10 px-4 py-2.5 text-sm font-semibold text-amber-100 hover:bg-amber-500/15"
            >
              <Crown className="h-4 w-4 text-amber-300" />
              {translate(
                language,
                "Premium để tạo group",
                "Premium to create groups",
              )}
            </Link>
          )}
        </div>
        ...
        {showCreate && (
          <form
            onSubmit={submit}
            className="surface mt-6 grid gap-4 p-5 sm:grid-cols-2"
          >
            <div className="sm:col-span-2">
              ...
              <p className="mt-2 text-sm text-slate-400">
                {translate(
                  language,
                  "Group public mở cho mọi người. Group private cần quyền Premium và sẽ luôn được API kiểm tra lại.",
                  "Public groups are open to everyone. Private groups require Premium and are always verified again by the API.",
                )}
              </p>
            </div>
            ...
            <label className="text-sm text-slate-300">
                ...
                onChange={(event) => {
                  ...
                  if (visibility === "private" && entitlement === "free") {
                    ...
                    setError(
                      translate(
                        language,
                        "Group riêng tư cần Premium. API cũng sẽ kiểm tra lại khi tạo group.",
                        "Private groups require Premium. The API will verify this again when creating the group.",
                      ),
                    );
                    return;
                  }
              ...
              {entitlement === "loading" ? (
                ...
                <span className="mt-1 block text-xs text-amber-200">
                  {translate(
                    language,
                    "Tài khoản Free chỉ tạo được group công khai. ",
                    "Free accounts can create public groups only. ",
                  )}
                  <Link
                    href="/account"
                ...
                <span className="mt-1 block text-xs text-emerald-200">
                  {translate(
                    language,
                    "Premium đang hoạt động: có thể tạo group riêng tư.",
                    "Premium is active: private groups are available.",
                  )}
                </span>
              ) : (
                <span className="mt-1 block text-xs text-slate-500">
                  {translate(
                    language,
                    "Quyền tạo group sẽ được API xác minh khi gửi.",
                    "The API will verify your entitlement when you submit.",
                  )}
                </span>
              )}
            </label>
            <label className="text-sm text-slate-300">
              ...
              <select
                ...
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    join_policy: event.target.value as
        ...
        {error && (
          <div
            role="alert"
            className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-100"
          >
            <span>{error}</span>
            <button
              type="button"
        ...
        {loading ? (
          ...
          <div className="mt-7 grid gap-4 sm:grid-cols-2">
            {groups.map((group) => (
              <Link
                ...
                className="surface group flex min-h-48 flex-col p-5 transition hover:border-sky-400/45"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-3">
                    <ExternalImage src={group.avatar_url} alt="" className="h-10 w-10 rounded-xl border border-slate-700 object-cover" fallback={<span className="grid h-10 w-10 place-items-center rounded-xl border border-sky-400/20 bg-sky-500/10 font-semibold text-sky-200">{group.name.slice(0, 1).toUpperCase()}</span>} />
                  ...
                  {group.visibility === "private" ? (
                    ...
                    <UsersRound className="h-4 w-4 shrink-0 text-sky-300" />
                  )}
                </div>
                ...
                <div className="mt-auto flex items-center justify-between pt-5 text-xs">
                  <span className="rounded-md bg-slate-800 px-2 py-1 text-slate-300">
                    {group.join_policy === "open"
                      ? translate(language, "Mở", "Open")
                      : group.join_policy === "approval"
                        ? translate(language, "Chờ duyệt", "Approval")
                        : translate(language, "Chỉ mời", "Invite only")}
                  </span>
                  <span className="inline-flex items-center gap-1 font-semibold text-sky-300 group-hover:text-sky-200">
                    {translate(language, "Xem group", "View group")}
          ...
          <div className="surface mt-7 p-10 text-center">
            ...
            <p className="mt-2 text-sm text-slate-400">
              {translate(
                language,
                "Hãy tạo group đầu tiên để bắt đầu một chủ đề thảo luận.",
                "Create the first group to start a discussion topic.",
              )}
            </p>
          </div>
        )}
