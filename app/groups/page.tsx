...
export default function GroupsPage() {
  ...
  return <main className="min-h-[calc(100vh-12rem)] px-4 py-8 sm:px-6 lg:py-12"><section className="mx-auto max-w-5xl">
    <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><div className="eyebrow"><UsersRound className="h-4 w-4 text-sky-400" />{translate(language, "Cộng đồng theo chủ đề", "Topic communities")}</div><h1 className="mt-2 text-3xl font-semibold text-white sm:text-4xl">{translate(language, "Group nhà đầu tư", "Investor groups")}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">{translate(language, "Thảo luận tập trung theo chủ đề, cùng quy tắc thành viên và luồng bài viết riêng.", "Focused discussion spaces with member rules and their own post feeds.")}</p></div>
      {!signedIn ? <Link href="/login" className="inline-flex items-center justify-center rounded-xl bg-sky-500 px-4 py-2.5 text-sm font-semibold text-slate-950 hover:bg-sky-400">{translate(language, "Đăng nhập để tạo group", "Sign in to create a group")}</Link> : entitlement === "premium" ? <button type="button" onClick={() => setShowCreate((current) => !current)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-sky-500 px-4 py-2.5 text-sm font-semibold text-slate-950 hover:bg-sky-400"><Plus className="h-4 w-4" />{translate(language, "Tạo group", "Create group")}</button> : entitlement === "loading" ? <span className="inline-flex items-center gap-2 rounded-xl border border-slate-700 px-4 py-2.5 text-sm font-semibold text-slate-400"><Loader2 className="h-4 w-4 animate-spin" />{translate(language, "Đang kiểm tra Premium…", "Checking Premium…")}</span> : <Link href="/account#premium" className="inline-flex items-center gap-2 rounded-xl border border-amber-400/35 bg-amber-500/10 px-4 py-2.5 text-sm font-semibold text-amber-100 hover:bg-amber-500/15"><Crown className="h-4 w-4 text-amber-300" />{translate(language, "Premium để tạo group", "Premium to create groups")}</Link>}</div>
