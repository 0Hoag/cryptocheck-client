    ...
    <main className="min-h-screen px-4 py-8 sm:px-6 lg:py-10">
      <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[1fr_300px]">
        <section>
          ...
          {loading ? <div className="grid place-items-center py-20" role="status"><Loader2 className="h-7 w-7 animate-spin text-sky-400" aria-hidden="true" /><span className="sr-only">{translate(language, "Đang tải thảo luận cộng đồng", "Loading community discussions")}</span></div> : posts.length ? <><div className="space-y-4">{posts.map((post) => <CommunityCard key={post.id} language={language} post={post} user={user} />)}</div>{hasMorePosts && <div className="mt-5 text-center"><button type="button" onClick={() => void loadMore()} disabled={loadingMorePosts} className="inline-flex items-center gap-2 rounded-lg border border-sky-400/30 px-4 py-2 text-sm font-semibold text-sky-200 hover:bg-sky-400/10 disabled:opacity-60">{loadingMorePosts && <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" />}{translate(language, "Xem thêm bài viết", "Load more posts")}</button></div>}{loadMoreError && <div role="alert" className="mt-4 rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-200"><p>{loadMoreError}</p><button type="button" onClick={() => void loadMore()} disabled={loadingMorePosts} className="mt-2 text-xs font-semibold text-red-100 underline underline-offset-2 disabled:opacity-60">{translate(language, "Thử lại", "Retry")}</button></div>}</> : <div className="surface p-10 text-center"><UsersRound className="mx-auto h-8 w-8 text-sky-400" /><p className="mt-4 font-medium text-white">{translate(language, "Chưa có thảo luận nào", "No discussions yet")}</p><p className="mt-2 text-sm text-slate-400">{translate(language, "Hãy mở đầu cuộc trò chuyện đầu tiên của cộng đồng.", "Start the community's first conversation.")}</p></div>}
        </section>
...
export function CommunityCard({ post, user, language }: { post: CommunityPost; user: AuthUser | null; language: "vi" | "en" }) {
  ...
  const visibilityLabel = post.permission === "followers"
  ...
  const loadInteractions = useCallback(async (): Promise<string | null | undefined> => {
    ...
    try {
      ...
      const reactionId = reactionData.find((item: Reaction) => item.author_id === user?.id)?.id || null;
  ...
  const like = async () => {
    ...
    try {
      if (priorReaction) {
        setMyReaction(null); setLikes((current) => Math.max(0, current - 1));
        await deleteReaction(priorReaction);
      } else {
        setMyReaction(`pending-like-${post.id}`); setLikes((current) => current + 1);
        const reaction = await createReaction(post.id); setMyReaction(reaction.id);
      }
    } catch (requestError) {
      setMyReaction(priorReaction); setLikes((current) => Math.max(0, current + (priorReaction ? 1 : -1)));
      setActionError(getErrorMessage(requestError, translate(language, "Không thể cập nhật lượt thích.", "Unable to update the like."))); setRetry("like");
    }
  ...
  const addComment = async (contentToSend = comment) => {
    ...
    setComments((current) => [...current, optimisticComment]); setComment("");
    try {
      const created = await createComment(post.id, trimmed);
      setComments((current) => current.map((item) => item.id === optimisticId ? created : item));
    } catch (requestError) {
      setComments((current) => current.filter((item) => item.id !== optimisticId)); setComment(contentToSend);
      setActionError(getErrorMessage(requestError, translate(language, "Không thể gửi bình luận.", "Unable to send this comment."))); setRetry("comment");
    }
  ...
  const share = async () => {
    ...
    const result = await shareLink({ title: translate(language, "Bài viết cộng đồng CryptoCheck", "CryptoCheck community post"), text: post.content.slice(0, 120), url });
