...
export default function PostDetail({
  ...
  const [interactionRetry, setInteractionRetry] = useState<
    "load" | "like" | "comment" | null
  >(null);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("cryptocheck-auth-change", syncSession);
  }, []);
    ...
    try {
      ...
      setLoadError(
        getErrorMessage(
          ...
          translate(
            language,
            "Không thể tải bài viết lúc này. Hãy thử lại.",
  ...
  async function submitReport() {
    ...
    if (reportReason.trim().length < 3) {
      setReportError(
        translate(
          language,
          "Hãy nhập lý do ít nhất 3 ký tự.",
    ...
    setReportError("");
    try {
      await apiClient.post("/api/v1/news-feed/reports", {
        target_type: "post",
        target_id: post.id,
      ...
      setReportError(
        getErrorMessage(
          ...
          translate(
            language,
            "Không thể gửi báo cáo lúc này. Hãy thử lại.",
    ...
    try {
      ...
      setInteractionError(
        getErrorMessage(
          ...
          translate(
            language,
            "Không thể tải tương tác của bài viết này.",
  ...
  const updatePostCount = (
    field: "reaction_count" | "comment_count",
    change: number,
  ) => {
    setPost((current) =>
      current
        ? { ...current, [field]: Math.max(0, (current[field] || 0) + change) }
        : current,
    );
  };
  ...
  const toggleLike = async () => {
    ...
    try {
      if (priorReaction) {
        setMyReaction(null);
        updatePostCount("reaction_count", -1);
        await deleteReaction(priorReaction);
      } else {
        setMyReaction(`pending-like-${post.id}`);
        updatePostCount("reaction_count", 1);
        const created = await createReaction(post.id);
        setMyReaction(created.id);
      }
    } catch (error) {
      setMyReaction(priorReaction);
      updatePostCount("reaction_count", priorReaction ? 1 : -1);
      setInteractionError(
        getErrorMessage(
          error,
          translate(
  ...
  const addComment = async (contentToSend = comment) => {
    ...
    setComment("");
    updatePostCount("comment_count", 1);
    try {
    ...
    return (
      ...
      >
        ...
        <span className="sr-only">
          {translate(language, "Đang tải bài viết", "Loading post")}
        </span>
      </div>
    );
  }
  ...
  if (!post)
    return (
      <main className="min-h-screen bg-[#050505] px-4 py-20 text-center text-white">
        <p role={loadError ? "alert" : undefined}>
          {loadError ||
            translate(language, "Không tìm thấy bài viết", "Post not found")}
        </p>
        {loadError && (
          <button
            type="button"
            onClick={() => void loadPost()}
            className="mt-5 rounded-lg border border-cyan-400/30 px-4 py-2 text-sm font-semibold text-cyan-200 hover:bg-cyan-400/10"
          >
  ...
  const postTitle =
    post.title || translate(language, "Bài viết cộng đồng", "Community post");
  const sourceName = post.source_url
    ? getSourceName(post.source_url)
      ...
      <div className="border-b border-white/5 bg-[#050505]">
        <div className="max-w-[1600px] mx-auto px-4 py-3 flex items-center gap-2 text-xs text-gray-500">
          <Link href="/" className="hover:text-white transition-colors">
            {translate(language, "Trang chủ", "Home")}
          </Link>
          <span>/</span>
          ...
          <div className="col-span-1 lg:col-span-8 xl:col-span-6">
            <article>
              ...
              <div className="flex items-center gap-4 text-xs text-gray-400 mb-8 border-b border-white/5 pb-6">
                ...
                {isCommunityPost && post.author && (
                  <Link
                    ...
                    className="inline-flex items-center gap-1.5 text-gray-300 hover:text-cyan-300 transition-colors"
                  >
                    <UserRound className="w-4 h-4" />
                    {post.author.username ||
                      translate(
                        language,
                        "Thành viên CryptoCheck",
                        "CryptoCheck member",
                      )}
                  </Link>
                )}
              ...
              {!isCommunityPost && (
                <aside
                  role="note"
                  className="mb-6 rounded-xl border border-sky-400/20 bg-sky-400/5 px-4 py-3 text-sm leading-6 text-slate-300"
                >
                  {translate(
                    language,
              ...
              <div className="mt-12 border-t border-white/10 pt-8">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <button
                    ...
                    className="flex items-center gap-2 text-sm text-gray-400 hover:text-white transition-colors bg-white/5 px-4 py-2 rounded-lg border border-white/5 hover:border-white/10"
                  >
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-sm">
                      {user ? (
                        <button
                          ...
                          disabled={interactionsLoading || liking}
                          ...
                          className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-2 transition-colors disabled:opacity-50 ${myReaction ? "bg-rose-500/10 text-rose-200" : "bg-white/5 text-gray-300 hover:bg-rose-500/10 hover:text-rose-200"}`}
                        >
                          <Heart
                            aria-hidden="true"
                            className={`w-4 h-4 ${myReaction ? "fill-current" : ""}`}
                          />
                          {post.reaction_count ||
                            translate(language, "Thích", "Like")}
                        </button>
                      ) : (
                        <Link
                          href="/login"
                          className="inline-flex items-center gap-1.5 rounded-lg bg-white/5 px-3 py-2 text-gray-300 hover:bg-white/10"
                        >
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (shouldOpen && !interactionsLoaded)
                            void loadInteractions();
                        }}
                        aria-expanded={commentsOpen}
                        aria-controls="post-detail-discussion"
                        aria-label={translate(
                          language,
                          "Mở bình luận",
                          "Toggle comments",
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    {getAuthToken() ? (
                      <button
                        ...
                        className="flex items-center gap-2 rounded-lg border border-white/10 px-3 py-2 text-sm text-gray-400 transition-colors hover:border-amber-400/30 hover:bg-amber-400/5 hover:text-amber-200"
                      >
                        <Flag className="w-4 h-4" />
                        {translate(language, "Báo cáo", "Report")}
                      </button>
                    ) : (
                      <Link
                        href="/login"
                        className="flex items-center gap-2 rounded-lg border border-white/10 px-3 py-2 text-sm text-gray-400 transition-colors hover:border-amber-400/30 hover:text-amber-200"
                      >
                        <Flag className="w-4 h-4" />
                        {translate(
                          language,
                ...
                {reportSuccess && (
                  <p
                    role="status"
                    className="mt-4 rounded-lg border border-emerald-400/20 bg-emerald-500/10 p-3 text-sm text-emerald-100"
                  >
                    {translate(
                      language,
                      "Đã gửi báo cáo. Đội ngũ kiểm duyệt sẽ xem xét.",
                      "Your report was submitted. The moderation team will review it.",
                    )}{" "}
                    <Link
                      href="/reports"
                      className="font-semibold text-sky-200 underline underline-offset-2 hover:text-sky-100"
                    >
                      {translate(language, "Xem trạng thái", "View status")}
                    </Link>
                  </p>
                )}
                {shareStatus === "success" && (
                  <p role="status" className="mt-4 text-sm text-emerald-200">
                    {translate(
                      language,
                      "Đã mở trình chia sẻ hoặc sao chép liên kết bài viết.",
                      "The share sheet was opened or the article link was copied.",
                    )}
                  </p>
                )}
                {shareStatus === "error" && (
                  <p role="alert" className="mt-4 text-sm text-red-200">
                    {translate(
                      language,
                      "Không thể chia sẻ liên kết lúc này. Hãy sao chép URL trên thanh địa chỉ.",
                      "Unable to share the link right now. Please copy the URL from the address bar.",
                    )}
                  </p>
                )}
                {isCommunityPost && commentsOpen && (
                  <section
                    id="post-detail-discussion"
                    aria-busy={interactionsLoading}
                    className="mt-5 border-t border-white/10 pt-5"
                  >
                    <h2 className="text-base font-semibold text-white">
                      {translate(language, "Thảo luận", "Discussion")}
                    </h2>
                    {interactionError && (
                      <div
                        role="alert"
                        className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-red-400/20 bg-red-500/10 p-3 text-sm text-red-100"
                      >
                        ...
                        {interactionRetry && (
                          <button
                            ...
                            disabled={
                              interactionsLoading || liking || commenting
                            }
                            className="font-semibold text-sky-200 underline underline-offset-2 disabled:opacity-50"
                          >
                            {translate(language, "Thử lại", "Retry")}
                          </button>
                        )}
                    ...
                    <div className="mt-4 space-y-3">
                      {interactionsLoading ? (
                        ...
                        comments.map((item) => (
                          <div
                            key={item.id}
                            className={`rounded-xl bg-white/5 p-3 text-sm ${item.id.startsWith("pending-comment-") ? "opacity-60" : ""}`}
                          >
                            <p className="font-medium text-slate-200">
                              {item.author_id === user?.id
                                ? user.username
                                : `${translate(language, "Nhà đầu tư", "Investor")} ${item.author_id.slice(-4)}`}
                            </p>
                            <p className="mt-1 whitespace-pre-wrap leading-6 text-slate-400">
                              {item.content}
                            </p>
                          </div>
                        ))
                    ...
                    {user ? (
                      ...
                      >
                        ...
                        <button
                          type="submit"
                          disabled={
                            interactionsLoading || commenting || !comment.trim()
                          }
                          aria-label={translate(
                            language,
                            "Gửi bình luận",
                          ...
                          className="inline-flex items-center rounded-lg bg-sky-500 px-3 text-sm font-semibold text-slate-950 disabled:opacity-50"
                        >
                          {commenting ? (
                            <Loader2
                ...
                {reportOpen && (
                  <section className="mt-4 rounded-xl border border-amber-400/20 bg-amber-400/5 p-4">
                    ...
                    <p className="mt-1 text-sm text-slate-400">
                      {translate(
                        language,
                        "Chỉ báo cáo nội dung vi phạm. Không dùng tính năng này cho tranh luận đầu tư thông thường.",
                        "Only report content that violates the rules. Do not use this for ordinary investment disagreements.",
                      )}
                    </p>
                    <label className="mt-4 block text-sm text-slate-300">
                      {translate(language, "Lý do", "Reason")}
                      <input
                        value={reportReason}
                        onChange={(event) =>
                        ...
                        minLength={3}
                        maxLength={250}
                        className="mt-1.5 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-white outline-none focus:border-amber-300"
                      />
                    </label>
                    <label className="mt-3 block text-sm text-slate-300">
                      ...
                      <textarea
                        ...
                        maxLength={1000}
                        rows={3}
                        className="mt-1.5 w-full resize-none rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-white outline-none focus:border-amber-300"
                      />
                    </label>
                    {reportError && (
                      <div
                    ...
                    <div className="mt-4 flex gap-2">
                      <button
                        ...
                        onClick={() => void submitReport()}
                        disabled={reporting || reportReason.trim().length < 3}
                        className="inline-flex items-center gap-2 rounded-lg bg-amber-300 px-4 py-2 text-sm font-semibold text-slate-950 disabled:opacity-50"
                      >
                        {reporting && (
                          <Loader2 className="h-4 w-4 animate-spin" />
                        )}
                        {translate(language, "Gửi báo cáo", "Submit report")}
                      </button>
                      <button
                        type="button"
